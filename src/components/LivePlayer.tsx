...
export default function LivePlayer({ channel }: LivePlayerProps) {
  ...
  const fetchStoredTracks = async () => {
    try {
      ...
      if (track) {
        ...
        console.log('🎵 No community tracks available, will generate new one');
      }
    } catch (error) {
  ...
  // Generate new track when no community tracks available
  const generateNewTrack = async () => {
    try {
      ...
      const response = await fetch('/api/music', {
        ...
        body: JSON.stringify({ prompt: `Live radio ${channel} vibes` })
      });
      
      const data = await response.json();
      if (data.taskId) {
        console.log('🎵 Generating new track for live radio...');
        // The track will be saved to database via callback
        // For now, we'll use a placeholder
        setCurrentTrackInfo({
          title: `Live ${channel} vibes`,
          prompt: `Live radio ${channel} vibes`,
          mood: channel
        });
      }
    } catch (error) {
  ...
  // Initialize with community tracks or generate new ones
  useEffect(() => {
    fetchStoredTracks();
  }, []);

  // Add overlay for now playing info
  const NowPlayingOverlay = () => {
    ...
    return (
      <motion.div
        ...
        className="absolute top-4 left-4 bg-black/50 backdrop-blur-lg rounded-xl p-4 border border-white/20 z-10"
      >
